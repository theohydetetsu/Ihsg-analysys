...
# CSS PREMIUM (DIPADATKAN & PRESISI)
st.markdown("""<style>
...
def get_stock_data(ticker_symbol):
    try:
        ...
        # 2. Multi-Timeframe Simulation (MTF Matrix berdasarkan tren 5 hari vs 20 hari vs 60 hari)
...
# ==========================================
# --- ALGORITMA PENILAIAN QUANT SKOR (MAX 13) ---
# ==========================================
score = 0
...
with col_h2:
    ...
    st.markdown(f"<div style='color:#fbbf24; font-size: 2.8rem; font-weight: 900; margin-top: 5px;'>🌟 {score}.0 <span style='font-size:1.4rem; color:#9ca3af;'>/13</span></div>", unsafe_allow_html=True)
...
# ==========================================
if score >= 10: entry_html = f"<span style='color:#4ade80;'>Sniper HK di <strong><span style='font-size:1.15rem;'>Rp{int(data['price'])}</span></strong></span>"
elif score >= 7: entry_html = f"<span style='color:#4ade80;'>Entry Optimal di <strong><span style='font-size:1.15rem;'>Rp{int(data['price'])}</span></strong></span>"
elif score >= 4: entry_html = f"<span style='color:#fbbf24;'>Antre Support di <strong><span style='font-size:1.15rem;'>Rp{int(data['sup'])}</span></strong></span>"
...
if risk_per_share > 0 and reward > 0:
    rr_html = f"<div style='background:#1e3a8a; color:white; padding:6px; border-radius:6px; text-align:center; font-weight:bold; font-size:0.8rem; margin-top: 6px;'>⚖️ Risk : Reward = 1 : {round(reward / risk_per_share, 1)}</div>"

if max_lot > 0 and score >= 4:
    mm_html = f"<div style='background:#065f46; color:#a7f3d0; padding:10px; border-radius:8px; text-align:center;'>🛒 MAKSIMAL BELI:<br><div style='font-size:1.8rem; font-weight:900; line-height:1.2; margin-top:3px;'>{max_lot} LOT</div></div>"
else:
    mm_html = "<div style='background:#7f1d1d; color:#fca5a5; padding:10px; border-radius:8px; text-align:center; font-weight:bold; font-size:1rem;'>🚫 KONDISI TIDAK AMAN UNTUK ENTRY</div>"
...
if "atas VWAP" in status_vwap: vwap_html = "VWAP: <span style='color:#4ade80; font-weight:bold;'>Di Atas VWAP (Kuat)</span>"
elif "di VWAP" in status_vwap: vwap_html = "VWAP: <span style='color:#fbbf24; font-weight:bold;'>Persis di VWAP</span>"
else: vwap_html = "VWAP: <span style='color:#f87171; font-weight:bold;'>Di Bawah VWAP</span>"

vpa_bg = "#065f46; color:#a7f3d0;" if data['vpa_score'] == 1 else "#78350f; color:#fde68a;"
vpa_html = f"<div style='background:{vpa_bg}; padding:4px 8px; border-radius:4px; margin-top:4px; display:inline-block; font-weight:bold; font-size:0.85rem;'>📊 VPA: {data['vpa_stat']}</div>"

if score >= 10: kesimpulan_html = f"⚡<br><div style='font-size:1.2rem; font-weight:900; color:#4ade80; margin: 4px 0;'>GOD MODE SNIPER!<br>{ticker_input} SIAP TERBANG</div><span style='color:#a3a8b4; font-size:0.85rem;'>Semua Engine Selaras.</span>"
elif score >= 7: kesimpulan_html = f"🔥<br><div style='font-size:1.2rem; font-weight:900; color:#4ade80; margin: 4px 0;'>{ticker_input} HIGH PROBABILITY</div><span style='color:#a3a8b4; font-size:0.85rem;'>Sinyal Sangat Kuat.</span>"
elif score >= 4: kesimpulan_html = f"⚠️<br><div style='font-size:1.2rem; font-weight:900; color:#fbbf24; margin: 4px 0;'>PANTAU KETAT {ticker_input}</div><span style='color:#a3a8b4; font-size:0.85rem;'>Sinyal Moderat.</span>"
else: kesimpulan_html = f"💀<br><div style='font-size:1.2rem; font-weight:900; color:#f87171; margin: 4px 0;'>JAUHI {ticker_input}</div><span style='color:#a3a8b4; font-size:0.85rem;'>Risiko Tinggi / Distribusi.</span>"

shares_str = f"{data['shares'] / 1e9:.2f} Miliar Lembar" if data['shares'] != "N/A" else "Tidak diketahui"

# Merakit Kotak (Tinggi Presisi 270px)
box1 = f"""<div style='height: 270px; display: flex; flex-direction: column;'>
<div><span style='font-size: 0.95rem; font-weight: bold;'>💰 TRADE PLAN & SNIPER</span><br><hr style='margin: 6px 0; border-color:#374151;'>{entry_html}<br><span style='color:#f87171;'><strong>Stop Loss:</strong> <span style='font-weight:bold;'>Rp{int(data['sup'])}</span></span></div>
<div style='margin-top: auto;'><hr style='margin: 6px 0; border-color:#374151;'>🎯 <strong>TARGET:</strong> TP1: Rp{int(data['res'])} | TP2: Rp{int(data['swing_high'])}<br>{rr_html}</div>
</div>""".replace('\n', '')

box2 = f"""<div style='height: 270px; display: flex; flex-direction: column;'>
<div><span style='font-size: 0.95rem; font-weight: bold;'>🛡️ MONEY MANAGEMENT</span><br><hr style='margin: 6px 0; border-color:#374151;'><span style='color:#9ca3af;'>Modal:</span> Rp{modal_input:,.0f} | <span style='color:#9ca3af;'>Risiko:</span> {risk_pct*100}% (<span style='color:#f87171;'>Rp{max_loss_rp:,.0f}</span>)</div>
...
<div><span style='font-size: 0.95rem; font-weight: bold;'>🦅 INSTITUTIONAL FLOW</span><br><hr style='margin: 6px 0; border-color:#374151;'>{asing_html}<br>{bandar_html}<br>{vwap_html}</div>
<div style='margin-top: auto;'><hr style='margin: 6px 0; border-color:#374151;'>{bb_stat}</div>
</div>""".replace('\n', '')

box5 = f"""<div style='height: 270px; display: flex; flex-direction: column;'>
<div><span style='font-size: 0.95rem; font-weight: bold;'>⏳ MULTI-TIMEFRAME MATRIX</span><br><hr style='margin: 6px 0; border-color:#374151;'><strong>Matrix:</strong><br><span style='font-size:0.75rem; color:#4ade80;'>{data['mtf_status']}</span><br><hr style='margin: 6px 0; border-color:#374151;'>📐 <strong>Fibo:</strong> {data['fibo_stat']}<br><span style='color:#fbbf24;'>Golden Ratio: Rp{int(data['fibo_618'])}</span></div>
<div style='margin-top: auto;'>{vpa_html}</div>
</div>""".replace('\n', '')

box6 = f"""<div style='height: 270px; display: flex; flex-direction: column;'>
<div><span style='font-size: 0.95rem; font-weight: bold;'>📊 TEKNIKAL & FUNDAMENTAL</span><br><hr style='margin: 6px 0; border-color:#374151;'>📌 <strong>EMA:</strong> {data['ema_cross']}<br>📌 <strong>RSI:</strong> {data['rsi_val']:.1f} ({data['rsi_status']})</div>
<div style='margin-top: auto;'><hr style='margin: 6px 0; border-color:#374151;'>👥 <strong>Saham Beredar:</strong><br><strong style='color:#f3f4f6;'>{shares_str}</strong></div>
</div>""".replace('\n', '')
